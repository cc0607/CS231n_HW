...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
      scores = X[i].dot(W)#每個類別的分數
      percentages = np.exp(scores)/np.sum(np.exp(scores))
      correct_class_percentage = percentages[y[i]]
      loss += -1*np.log(correct_class_percentage)
      dscores = percentages.reshape(1,-1)
      dscores[:, y[i]] -= 1
